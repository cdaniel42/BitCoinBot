...
#    cnn = ConvNet.ConvNet(seqLength, predLength)
    
#    x = np.linspace(0, 100, int(1e4) )
#    y = np.sin(x) + np.random.randn(x.shape[0]) * 0.02 + 15
    #%%
    
    dataCSV = pandas.read_csv(
            "./bitcoin-historical-data/bitstampUSD_1-min_data_2012-01-01_to_2017-10-20.csv",
            verbose=False, ) 
    ...
    # Train the model
        ...
        yPred = [ p["values"][21] for p in predRes]
        plt.figure(1)
        plt.clf()
        plt.plot(targetsTest[-50000:,21])
        plt.plot(yPred, 'r.')
